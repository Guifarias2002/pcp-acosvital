...
function PedidosPageInner() {
  ...
  function buscar() {
    setLoading(true);
    // Inclui pedidos entregues se o filtro de etapa for entregue
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            ...
            {!loading && pedidos.filter(p => !fEtapa || getPedidoEtapa(p) === fEtapa).length === 0 && (
              <tr><td colSpan={8} style={{ textAlign: 'center', padding: 40, color: '#999' }}>Nenhum pedido encontrado.</td></tr>
            )}
            {pedidos.filter(p => !fEtapa || getPedidoEtapa(p) === fEtapa).map(p => (
              <tr key={p.id} style={{ borderBottom: '1px solid #f0f0f0', background: p.atrasado ? '#fff8f8' : undefined }}>
