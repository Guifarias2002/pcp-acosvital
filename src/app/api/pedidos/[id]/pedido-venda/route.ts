...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const rows = await sql`SELECT pedido_venda_url FROM producao_pedido WHERE id = ${pedidoId}`;
    const storagePath: string | null = rows[0]?.pedido_venda_url ?? null;

    if (!storagePath) return new Response('Sem pedido de venda anexado', { status: 404 });

    // Arquivo novo (Backblaze) — marcado com prefixo "b2:"
    if (storagePath.startsWith('b2:')) {
      const r = await b2Download(storagePath.slice(3));
      if (!r.ok) {
        console.error('[pedido-venda] B2 download falhou', r.status);
      ...
      return new Response(r.body, {
        headers: {
          ...
          'Content-Disposition': `inline; filename="pedido_venda_${pedidoId}.${extB2}"`,
    ...
    console.error('[pedido-venda] erro ao baixar:', e);
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const fileName = `pedido_${pedidoId}_pv.${ext}`;
    ...
    await sql`UPDATE producao_pedido SET pedido_venda_url = ${'b2:' + fileName} WHERE id = ${pedidoId}`;
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    await sql`UPDATE producao_pedido SET pedido_venda_url = NULL WHERE id = ${pedidoId}`;

    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error('[pedido-venda] erro ao remover:', e);
    return NextResponse.json({ erro: 'Erro ao remover pedido de venda' }, { status: 500 });
  }
}
