      ...
      if (parcial.setor_atual === 'usinagem') {
        const [{ resta }] = await tx`
          SELECT EXISTS(
            ...
            WHERE i2.pedido_id = ${parcial.pedido_id} AND pa.setor_atual = 'usinagem'
              AND pa.status IN ('em_aberto','recebido','em_andamento','pausado')
          ) AS resta`;
        if (!resta) await tx`UPDATE producao_pedido SET previsao_conclusao = NULL, atualizado_em = NOW() WHERE id = ${parcial.pedido_id}`;
      }
    });
