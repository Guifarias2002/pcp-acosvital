...
// opcional de período por mês { de: 'YYYY-MM', ate: 'YYYY-MM' } e com_itens=1
// (anexa os produtos de cada pedido — usado na exportação "com produto").
...
// Registra o sinete do MATERIAL informado (item_pedido_id) e devolve esse
// material pro destino (qualidade/acabamento). Cada material tem sua própria
// observação de sinete — por isso o item_pedido_id é obrigatório. Sem ele
// (legado) o back grava em todos os itens do pedido no Sinete.
export const registrarSinetePedido = (pedidoId: number, body: { texto?: string; destino: string; semSinete?: boolean; item_pedido_id?: number }) =>
