...
export interface OPLeitura { materiais: OPMaterial[]; roteiro: OPOperacao[]; confianca: number; paginas: number; }
...
async function extractPages(buf: Buffer): Promise<Line[][]> {
...
export async function lerOP(buf: Buffer): Promise<OPLeitura> {
  ...
  return { materiais, roteiro, confianca, paginas: pages.length };
}
